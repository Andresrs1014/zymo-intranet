...
// Previsualiza un adjunto usando un blob URL autenticado
function AttachmentPreview({ attachment }: { attachment: TaskAttachment }) {
  const { blobUrl, loading, error } = useAttachmentBlobUrl(attachment.id)
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-3 p-8">
      <span className="text-5xl">{fileIcon(attachment.mime_type)}</span>
      <div className="text-center">
        <p className="font-medium text-gray-900">{attachment.filename}</p>
        <p className="text-sm text-gray-500 mt-0.5">{formatBytes(attachment.size_bytes)}</p>
      </div>
      <a
        href={blobUrl}
...
export function AttachmentExplorer({ taskId, taskTitulo, open, onClose }: Props) {
  const { data: attachments = [] } = useTaskAttachments(open ? taskId : null)
              ...
              {attachments.map((a) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5">{formatBytes(a.size_bytes)}</p>
                  </div>
