...
import {
  usePartnerProspectos, usePartnerCreateProspecto, usePartnerUpdateProspecto, usePartnerDeleteProspecto,
} from "@/hooks/useLibertadoraPartner"
import { ProspectosTable } from "@/components/libertadora/prospectos/ProspectosTable"
import { ProspectoFormDialog } from "@/components/libertadora/prospectos/ProspectoFormDialog"
import type { LibProspecto, LibProspectoInput } from "@/types/libertadora"

function exportCsv(rows: LibProspecto[]) {
...
export function PartnerProspectosPanel() {
  const { data: prospectos, isLoading, isError } = usePartnerProspectos()
  const createMut = usePartnerCreateProspecto()
  const updateMut = usePartnerUpdateProspecto()
  const deleteMut = usePartnerDeleteProspecto()
