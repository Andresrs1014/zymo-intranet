"""Creación de reuniones de Microsoft Teams para Agenda T&C, vía Microsoft
Graph API (client credentials — app-only, sin login del líder).
...
def crear_reunion(
    ...
    hora_inicio: str,
    hora_fin: str,
    correos_asistentes: list[str],
) -> dict | None:
    """Crea un evento de calendario con reunión de Teams para el organizador
    configurado, invitando a `correos_asistentes` (solo quienes tengan correo
    corporativo — se filtra antes de llamar esta función). Retorna
    {"event_id", "join_url"} o None si falla (nunca lanza)."""
    ...
    payload = {
        ...
        "attendees": [
            {"emailAddress": {"address": correo}, "type": "required"}
            for correo in correos_asistentes
        ],
    }
